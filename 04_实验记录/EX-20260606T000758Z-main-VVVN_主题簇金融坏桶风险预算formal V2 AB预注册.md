...
2026-06-08 剩余两段 formal 预执行后台豁免：计划运行 `${QUANT_PLATFORM_ROOT}/scripts/research/run_r010theme_vvvn_remaining_formal.sh`，顺序执行 `baseline`、`financial_top1_cap70`、`financial_or_share_ge050_cap70` 在 `2020_2021` 和 `2022_2023` 两段的 6 条预注册配置。后台原因是两段各覆盖两年、预计耗时较长；后台只用于主控轮询过程日志，不改变回测配置、不更新数据。进程标识：Windows PID `12028`，通过 ASCII junction `E:\quant_platform_v1_4_0_link` 进入同一平台目录；日志路径：`${QUANT_PLATFORM_ROOT}/results/v2/research/R010-THEME/EX-20260606T000758Z-main-VVVN/risk_budget/logs/formal_remaining/20260607T221500Z/`；查看进度：`wsl -- bash -lc "cd /mnt/e/quant_platform_v1_4_0_link && tail -n 40 results/v2/research/R010-THEME/EX-20260606T000758Z-main-VVVN/risk_budget/logs/formal_remaining/20260607T221500Z/*.run.log"`；停止方式：`Stop-Process -Id 12028` 或 `wsl -- bash -lc "pkill -f run_r010theme_vvvn_remaining_formal.sh; pkill -f 'EX-20260606T000758Z-main-VVVN.*run_v2_backtest.py'"`；预计耗时：60-120 分钟。此前两次 `nohup` 尝试未产生进程或日志，不计入有效回测。
```
...
E:/量化平台_V1.4.0/results/v2/research/R010-THEME/EX-20260606T000758Z-main-VVVN/risk_budget/summary/metrics_vs_baseline.csv
E:/量化平台_V1.4.0/results/v2/research/R010-THEME/EX-20260606T000758Z-main-VVVN/risk_budget/summary/theme_budget_records.csv
```
