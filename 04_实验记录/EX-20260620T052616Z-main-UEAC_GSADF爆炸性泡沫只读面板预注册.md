...
扩展事件源到 A11-2020 版，重跑生成 361 事件面板（342/361 有效 GSADF）。结果目录 `ueac_2020_panel/`。

| 指标 | 212 版 | 361 版 | 变化 |
| --- | --- | --- | --- |
| 事件数（高/低爆炸性桶） | 42/43 | 69/69 | **过 50 门槛** |
| 高 vs 低 effect 差 H10 | -0.037 | **+0.005** | 方向从"反"变"微正"，接近无差异 |
| 高爆炸性 H10 胜率 | 50% | 52.2% | 刚过 52% 门槛 |
| 分段一致性 | 1/1 | 2/3 | 略改善但仍不足 |

**结论更新**：GSADF 在 361 版方向不再明显相反（差 +0.005 接近 0），变成无差异而非反转。但仍不支持作为过热撤离信号（无显著预测力）。纯 numpy 简化 ADF 的不确定性仍在。

**判断维持 observe（方向无差异）**：GSADF 不纳入 p_overheat，作方法资产保留。
