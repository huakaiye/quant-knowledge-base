...
status: completed
stage: formal_delay_negative_control_strict_20of20_no_promote
owner: main
created_at: 2026-06-06T14:07:52Z
updated_at: 2026-06-07T20:49:18+08:00
...
summary_paths:
  ...
  - results/v2/research/R010-TOPN/EX-20260606T140752Z-main-TC8U/summary_delay_negative_control/summary.json
  - scripts/research/summarize_tc8u_topn_antishake.py
quality_gate: L2_formal_cost_completed_random_delay_negative_control_confirmed_no_promote
subagent_call_ids:
...
下游 C35J 已完成切换延迟负控：固定 seed 的 `random50` 能在 3/4 分段复制并超过 Top3/gap，说明 TopN/gap 的机制解释被普通延迟混淆，不能 promote。TC8U 自身的 `formal_delay_negative_control` 已完成 20/20 strict：`delay_random050_no_topn` 相对 baseline 为 3/4 分段 final 不低、3/4 分段 MDD 不差、交易数 4/4 更低，四段 final 合计多 `88976.39`；但 `top3_gap_confirm1_veto` 只有 1/4 分段 final 不低，四段合计少 `69701.83`。
下一步要做：不再扩大 TopN 网格，不把随机延迟、confirm1 或 Top3/gap 写入默认交易逻辑。若要叠加 A23，必须新开组合交互实验。
...
| 已做消融或负控 | 下游负控完成，TC8U 自身负控 strict 强化 | baseline 与邻近收紧完成；C35J `confirm1/random50` 16/16 strict 完成并触发 observe；TC8U `formal_delay_negative_control` 已完成 20/20 strict，其中随机延迟四段完整并进一步强化负控 |
...
python scripts/research/summarize_tc8u_topn_antishake.py --phase formal_delay_negative_control --variants baseline_top1_hard5 top3_ratio070_gap050_veto delay_confirm1_no_topn delay_random050_no_topn top3_gap_confirm1_veto --output-dir results/v2/research/R010-TOPN/EX-20260606T140752Z-main-TC8U/summary_delay_negative_control --strict
```
...
results/v2/research/R010-TOPN/EX-20260606T140752Z-main-TC8U/summary_delay_negative_control/
```
...
TC8U 自身 `formal_delay_negative_control` strict 结果：

- 当前完成 `20/20` strict：`expected_runs=20`、`completed_runs=20`、`missing=[]`。
- `delay_random050_no_topn` 相对 baseline 为 3/4 分段 final 不低、3/4 分段 MDD 不差、4/4 分段交易数更低，四段 final 合计多 `88976.39`，交易少 `309`。
...
- `top3_gap_confirm1_veto` 2025_20260519 final `296133.62`，相对 baseline 少 `15907.04`；四段合计 final 差为 `-69701.83`，只有 1/4 分段 final 不低，确认延迟组合正式失败。
...
- `formal_delay_negative_control` strict 通过：`expected_runs=20`、`completed_runs=20`、`missing=[]`。
...
- TC8U 自身 `formal_delay_negative_control` 已完成 20/20 strict：随机延迟四段完整且明显强于 Top3/gap，但没有 TopN 机制含义；`top3_gap_confirm1_veto` 只有 1/4 分段 final 不低，四段合计 final 少 `69701.83`，确认延迟组合正式失败。
...
理由：TopN 抗抖证明了“减少换仓”这个执行效果，也证明 Top3/gap 和 Top5 在两类成本扰动下没有因成本加倍而整体崩掉。但它仍没有证明“稳定提升收益”：Top5 在 2020/2022 两段持续低于 baseline；Top3/gap 在 2024 明显低于 baseline 且 MDD 稳定性不足。下游 C35J 又显示普通随机延迟能在 3/4 分段复制并超过 Top3/gap；TC8U 自身 strict 也显示随机延迟合计 final 多 `88976.39`，而 `top3_gap_confirm1_veto` 合计少 `69701.83`。机制解释被负控进一步压制。因此不改默认 hard5/A23，已同步 [[05_研究决策/DEC-20260607T095915Z-main-BP3L_TopN抗抖随机延迟负控后观察决策|BP3L observe 决策]]。

## 17. 下一步

1. 不继续扩大 TopN 排名、比例、分差、seed 或冷却期网格；TopN/gap、confirm1、random50 均不进入默认交易逻辑。
2. 若继续方向 4，只能新开“换仓确认机制”预注册，并先做 seed 敏感性、2022 失败归因、成本扰动和错位负控。
3. A23 或 hard5 默认逻辑不因 TC8U/C35J 改动；A23 组合交互必须另开实验。
