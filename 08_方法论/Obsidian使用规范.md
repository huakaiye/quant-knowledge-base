...
Obsidian Git 自动同步配置见 `08_方法论/Obsidian Git自动同步配置.md`。本地可以启用自动保存和自动备份，但跨端拉取、合并和推送默认交给 `tools/Sync-ResearchRepo.ps1`；`.obsidian/plugins/obsidian-git/data.json` 仍不入库。
